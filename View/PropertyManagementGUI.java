...
public class PropertyManagementGUI extends JFrame {
    ...
    public PropertyManagementGUI() { this("Landlord"); }
    ...
    private JPanel createRoomPanel(boolean showAddForm) {
        ...
        r1.add(new JLabel("Description:")); r1.add(desc);
        r1.add(new JLabel("Room #:"));      r1.add(num);

        JPanel r2 = new JPanel(new FlowLayout(FlowLayout.LEFT));
        r2.add(new JLabel("Rent Min:")); r2.add(min);
        r2.add(new JLabel("Rent Max:")); r2.add(max);

        JPanel r3 = new JPanel(new FlowLayout(FlowLayout.LEFT));
        r3.add(new JLabel("Type:")); r3.add(type);
        r3.add(avail); r3.add(search); r3.add(reset);

        wrap.add(r1); wrap.add(r2); wrap.add(r3);
        panel.add(wrap, BorderLayout.NORTH);

        search.addActionListener(e -> applyRoomFilter(desc,num,min,max,type,avail));
        reset .addActionListener(e -> {
            desc.setText(""); num.setText(""); min.setText(""); max.setText("");
            type.setSelectedIndex(0); avail.setSelected(false); refreshUI();
        });

        // ------- Landlord-only “Add room” form -------
        if (showAddForm) {
            ...
            JLabel specL = new JLabel("Special:"); form.add(specL); form.add(specF);
            ...
            typeC.addActionListener(e -> {
                ...
                specF.setVisible(show); specL.setVisible(show);
    ...
    private JPanel createTenantPanel() {
        ...
        crud.add(new JLabel("Name:"));   crud.add(name);
        crud.add(new JLabel("Credit:")); crud.add(credit);
        crud.add(new JLabel("Phone:"));  crud.add(phone);
        crud.add(add); crud.add(del);
    ...
    private JPanel createLandlordPanel() {
        ...
        JButton add = new JButton("Add"),
                del = new JButton("Delete");
        in.add(new JLabel("Name:")); in.add(name);
        in.add(add); in.add(del);
        p.add(in, BorderLayout.SOUTH);
    ...
    private JPanel createLeasePanel() {
        ...
        // ——— store in fields so refreshUI() can touch them ———
        ...
        in.add(new JLabel("Room:"));    in.add(roomComboBox);
        in.add(new JLabel("Tenant:"));  in.add(tenantComboBox);
        in.add(new JLabel("Landlord:"));in.add(landlordComboBox);
        in.add(new JLabel("Balance:")); in.add(bal);
        in.add(create); in.add(end);
        p.add(in, BorderLayout.SOUTH);
        ...
        create.addActionListener(e -> {
            try {
                ...
                r.setAvailable(false); database.updateRoomAvailability(r);
                reloadData(); refreshUI();
            } catch (Exception ex) {
                JOptionPane.showMessageDialog(this,"Bad data");
            }
            ...
            if (sel>=0) {
                ...
                r.setAvailable(true); database.updateRoomAvailability(r);
                reloadData(); refreshUI();
            }
        });
    ...
    // refresh the three combo-boxes in the Lease panel
    ...
    private void refreshUI() {
        ...
        // NEW: keep Lease combos up-to-date
        updateLeaseCombos();
    }
